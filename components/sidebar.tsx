...
export default function Sidebar() {
    ...
    const [tabObjs, setTabObjs] = useState<Tabs[]>([{ name: 'new tab', url: '', presentId: 1 }]);
    const [tabID,setTabID] = useState<number>(1);
    const addTab = () => {
        const newTabID = tabID + 1;
        setTabObjs(prevTabObjs => [...prevTabObjs, { name: 'new tab', url: '', presentId: newTabID }]);
        setTabID(newTabID);
    }
    return (
            ...
            <div>
                ...
                {tabObjs.map((tab)=>{
                    ...
                    if(tab.presentId!=tabID){
                        return(                            
                        <Card className={`m-3 justify-items-center text-white ${defaultTabStyle}`}>
